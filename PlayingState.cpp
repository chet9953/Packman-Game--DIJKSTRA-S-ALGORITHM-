...
string PlayingState::player_name = "Anonymous Player";
const string PlayingState::leaderboardFilePath = "leaderboard.txt";
...
map<string, int> PlayingState::loadLeaderboard() const
{
	...
	try
	{
		...
		while (file >> name >> score) leaderboard[name] = score;
	}
	catch(...) {
		debug.logLine("Couldn't read the leaderboard, so let's create it now!");
		ofstream ofile(leaderboardFilePath);
		ofile.close(); // create and close the leaderboard file
	}
...
void PlayingState::init()
{
	...
	blinky->teleport(GHOST_START_X, GHOST_START_Y, true); // spawn blinky in game
	...
	textDisplay.init(loadLeaderboard());
	showText = true;

	debug.logLine("Init done...");
}

void PlayingState::loop() // game logic
{
	Time::tick();
	
	//debug.logLine("Total Seconds: " + to_string(elapsed_total));
	//debug.logLine("Current Elapsed: " + to_string(elapsed));

	if (pacmanCanMove() && pacman->isAlive()) pacman->move(Time::elapsed); // if pacman can move, move it
	...
	if (maze->removeDot(pacman, ghosts)) // check if pacman just ate a dot, and if he did, remove the dot from screen
	{
		textDisplay.increase_score();
	}
	...
	for (Ghost* ghost : ghosts) teleportTunnels(ghost);
	...
	if(pacman->getDotsEaten() >= WINNING_SCORE) // we won
	{
		...
		waitTime++; // add another frame to waiting time (we wait so that we don't restart game right away)
	}

	if (!pacman->isAlive()) waitTime++; // player died so game is over
...
void PlayingState::draw(sf::RenderWindow& window) const // draws everything
{
	sf::Sprite pacmanSprite, blinkySprite, pinkySprite, inkySprite, clydeSprite; // gets all the sprites
	...
	for (int x = 0; x < Maze::SIZE_X; x++)
	{
		...
		{
			sf::Sprite *piece = Resources::mazePieces[maze->getTileCode(x, y)]; // get the maze piece
...
void PlayingState::keyPressed(int code) // handle keypress events
{
	using namespace sf;
	...
	case Keyboard::D:
		...
		break;
	case Keyboard::T:
		showText = !showText; // toggle whether we display the text or not
		break;
	}
}
...
void PlayingState::update_player_score(int score)
{
	ofstream file(leaderboardFilePath, ios_base::app | ios_base::out);
	file << player_name + " " + to_string(score) << endl;
}

bool PlayingState::pacmanCanMove() const // can pacman move?
{
...
void PlayingState::handleGhostMovement(Ghost* ghost)
{
	...
	if (ghostCanMove(ghost) && ghost->isOutOfCage()) // if ghost can move and is out of cage
		ghost->move(Time::elapsed); // then move
	else ghost->setTakeDecision(true); // otherwise, time to make a decision
}

bool PlayingState::ghostCanMove(Ghost* ghost) const // can the ghost move?
{
...
void PlayingState::teleportTunnels(Creature* creature) // if a creature stands in a tunnel, teleport it to the other end
{
...
void PlayingState::handleGhostCollision(Ghost* ghost)
{
...
sf::Sprite PlayingState::getGhostSprite(Ghost * ghost) const // gets appropriate sprite for this ghost
{
