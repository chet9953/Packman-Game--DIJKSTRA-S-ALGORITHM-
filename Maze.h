...
private:
	...
	bool is_inside_borders_x(int x) const; // is this x inside borders?
	bool is_inside_borders_y(int y) const; // is this y inside borders?
...
public:
	...
	bool is_inside_borders(int x, int y) const; // is this point inside the borders?
	...
	bool removeDot(PacMan* pacman, std::vector<Ghost*> ghosts); // this tile previously held a dot (the coin), turn it to normal
};