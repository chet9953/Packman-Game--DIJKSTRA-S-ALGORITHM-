...
bool Maze::is_inside_borders_x(int x) const
{
	return x >= 0 && x < SIZE_X;
}

bool Maze::is_inside_borders_y(int y) const
{
	return y >= 0 && y < SIZE_Y;
}
...
bool Maze::is_inside_borders(int x, int y) const
{
	return is_inside_borders_x(x) && is_inside_borders_y(y);
}
...
bool Maze::removeDot(PacMan* pacman, std::vector<Ghost*> ghosts)
{
	if (!is_inside_borders_x(pacman->getGridX() || !is_inside_borders_y(pacman->getGridY()))) return false;
