...
:Creature(gridX, gridY), name(name), destinationGridX(destinationX), destinationGridY(destinationY), 
	startX(gridX), startY(gridY), startDestX(destinationX), startDestY(destinationY), spriteID(resourceID)
{
	scattering = true;
	outOfCage = false;
	decision = true;
...
void Ghost::reset() 
{
	gridX = startX;
	gridY = startY;
	destinationGridX = startDestX;
	destinationGridY = startDestY;
}

void Ghost::move(float elapsed)
{
	float speed = MOVE_SPEED * elapsed;
